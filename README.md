...
- `label_matrix`: draw each side name plus axis direction in the top-left corner of its matrix tile using Pillow
...
- `view_names`: newline-separated labels such as `front (-Y)`, `right (+X)`, `top (+Z)` for Z-up models
